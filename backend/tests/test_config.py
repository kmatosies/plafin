...
def test_settings_accept_canonical_production_names(
    ...
    assert settings.stripe_price_pro_monthly_usd == "price_usd_placeholder"
    assert settings.resend_api_key == "resend-placeholder"
...
def test_optional_integrations_do_not_block_boot() -> None:
    ...
    assert settings.stripe_price_pro_monthly_usd == ""
    assert settings.enable_notification_worker is False
